...
############## plot specific for lab presentation

fPS18S
fPS18slv

ps18 <- subset_taxa(fPS18S, family%in%"Eimeriidae")
#ps18 <-aggregate_taxa(ps18, level="family")
#ps18 <- psmelt(ps18)
#ps18$logA <- log(1+ps18$Abundance)
#ps18$logGC <- log(1+ps18$Genome_copies_gFaeces)

ps18s <- subset_taxa(fPS18slv, Family%in%"Eimeriorina")
...
get_taxa_unique(ps18, "species")

get_taxa_unique(ps18s, "Species")

print(cor.test(ps18s$logGC, ps18s$logA, method="pearson"))
print(cor.test(ps18$logGC, ps18$logA, method="pearson"))

# Linear models
slv_plot <- ggplot(eim_slv, aes(y=logGC, x=logA))+
    ...
    xlab("Eimeriorina (log1+)")+
    ggtitle("18S Silva132-db")+
#    labs(tag= "a)")+
...
print(cor.test(eim_bla$logGC, eim_bla$logA, method="pearson"))
...
# Linear models
blast_plot <- ggplot(eim_bla, aes(y=logGC, x=logA))+
    geom_jitter(shape=21, position=position_jitter(0.2), size=4, aes(fill= dpi), color= "black", alpha=0.7)+
    geom_smooth(method = "lm", se=FALSE) +
    stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")),parse = TRUE)+
    ylab("Genome copies gFaeces(log 1+)")+
...
ggplot2::ggsave(file="fig/corr_blastEim18S.pdf", blast_plot, width = 7, height = 5, dpi = 300)
ggplot2::ggsave(file="fig/corr_slvEim18S.pdf", slv_plot, width = 7, height = 5, dpi = 300)
